...
public class Bean {
    ...
    // Integer semester = LocalDate.now().getMonthValue() <= 6 ? 1 : 2; // Học kỳ 1
    // hoặc 2
    // Integer year = LocalDate.now().getYear();

    // // Lưu bản ghi cấp phát mới (không cần kiểm tra điều kiện)
    // PageAllocation newAllocation = new PageAllocation(semester, year,
    // LocalDate.now(), defaultPages, true);
