...
public class HomeController {
    ...
    // @PostMapping("/login")
    // public LoginResponse login_proccess(@RequestBody User user) throws
    // LoginException {
    ...
    public ResponseEntity<ApiResponse> loginStudent(@RequestBody Student student) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ApiResponse("error", "Student không tồn tại"));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    // @PostMapping("/register")
    // public Student registerStudent(@RequestBody StudentRegistrationRequest
    // request) {
    // try {
    ...
    // }
    @PostMapping("/register")
    public ResponseEntity<ApiResponse> registerStudent(@RequestBody StudentRegistrationRequest request) {
        try {
            if (studentService.isUsernameTaken(request.getUsername())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse("error", "Username đã tồn tại. Vui lòng chọn username khác."));
            }
            if (studentService.isStudentNumberTaken(request.getStudentNumber())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse("error", "Student Number đã tồn tại."));
            }
            Student student = new Student(
                    request.getFirstName(),
                    request.getLastName(),
