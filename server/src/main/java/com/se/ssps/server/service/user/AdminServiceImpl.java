...
public class AdminServiceImpl implements AdminService {
    ...
    // @Override
    // public PageUnitPrice setPagePrice(Integer pagePrice) {
    // PageUnitPrice existing =
    // pageUnitPriceRepo.findAll().stream().findFirst().orElse(null);
    ...
    // List<PrintingLog> logs = printingLogRepository.findAllLogsBetween(fromDate,
    // toDate);
    ...
    // .collect(Collectors.groupingBy(log -> log.getPrinter().getId(),
    // Collectors.counting()));
    ...
    // : "Room ko xác định";
    // String buildingName = printer.getBuilding() != null ?
    // printer.getBuilding().getBuildingName()
    ...
    // // Xử lý thống kê
    // Double requestCount = requestCounts.getOrDefault(printer.getId(),
    // 0L).doubleValue();
    ...
    // // Lấy số lần yêu cầu in cho từng máy in từ cơ sở dữ liệu
    // List<Map<String, Object>> requestCounts =
    // printingLogRepository.countPrintingRequests(fromDate, toDate);
    ...
    // // Tìm số lần yêu cầu in của máy in này trong danh sách `requestCounts`
    // Optional<Map<String, Object>> requestCountEntry = requestCounts.stream()
    // .filter(entry -> entry.get("_id").equals(printer.getId())) // Kiểm tra
    // printerId
    // .findFirst();

    // Long requestCount = 0L;
    // if (requestCountEntry.isPresent()) {
    // requestCount = (Long) requestCountEntry.get().get("count");
    // }
    ...
    // PageUnitPrice firstPageUnitPrice = pageUnitPriceList.isEmpty() ? new
    // PageUnitPrice()
    // : pageUnitPriceList.get(0);

    // // Tính lợi nhuận
    // Double profit = pageNum * firstPageUnitPrice.getPrice().doubleValue(); //
    // Chuyển Integer thành Double
    ...
    // List<PrintingLog> printingLog =
    // printingLogRepository.findByDateRange(fromDate, toDate);
    ...
    // pageUnitRepo.findByPageSize(log.getPageSize().toString());
    // if (pageUnitPrice == null) {
    // throw new RuntimeException("Không tìm thấy giá cho kích thước giấy: " +
    // log.getPageSize());
    // }
    ...
    // List<PrintingLog> result =
    // printingLogRepository.calculateTotalPagesByType(fromDate, toDate);
